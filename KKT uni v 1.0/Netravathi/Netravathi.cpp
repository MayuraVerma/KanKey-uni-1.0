...
void processGunitaksharas(int ch,int no_bs,int i,int glp_pos)
{
								  switch(ch)
                                      {	
                                                  case 'a':
...
BOOL EXPORTED_DLL_FUNCTION InstallKeyboardHookNetravathi(HWND hWnd) 
{
          BOOL bSuccess = FALSE;
          if( !ghKeyHook) 
          {
...
long __stdcall InstallKeyboardHookNetravathiDEF() 
{
//          InstallKeyboardHookNetravathi(hwnd);
                    glpfnHookProc = (HOOKPROC) SetKeyBoardNetravathi;
...
long __stdcall DeInstallKeyboardHookNetravathiDEF() 
{
...
BOOL EXPORTED_DLL_FUNCTION DeInstallKeyboardHookNetravathi()
{
          if(ghKeyHook) 
          {
		  ...
		  if(!kFlag.found)	
			{
			  ...
			  if(ch == vowels[i].key)		// ---search the structure for the key pressed
			   { 
                              ...
                              { 
								  processGunitaksharas( ch, no_bs, i, glp_pos);
								  kFlag.n_back = -1;
                              }
